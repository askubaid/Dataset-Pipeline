...
CONFIG = {
    ...
    # Total sentence pairs to generate per theme
    "SENTENCES_PER_THEME": 1160,

    # Batch sizes (pairs requested per LLM call)
    "API_BATCH_SIZE": 30,
    "LOCAL_BATCH_SIZE": 20,
    ...
    # Output files
    "FILE_SOURCE": "learner.en",
    "FILE_TARGET": "learner.pt",
}
# ==========================================
...
class SentencePair(BaseModel):
    source: str = Field(description=f"Sentence in the source language")
    target: str = Field(description=f"Sentence in the target language")
...
def get_api_client(engine):
    ...
    if engine == "VERTEX":
        # Safe, card-protected enterprise track using your $1,000 credit
        return genai.Client(vertexai=True)
    
    elif engine == "API":
        # Standard developer-tier track using your personal AI Studio key
        load_dotenv()
        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key or api_key == "your_api_key_here":
...
# def generate_via_api(client, prompt, batch_size):
#     """Call Gemini API and return a list of {source, target} dicts."""
#     try:
...
def generate_via_api(client, prompt, engine):
    ...
    # Select the ideal model depending on your engine
...
def generate_via_local(prompt):
    """Call local Ollama model and return a list of {source, target} dicts."""
    payload = {
...
def build_prompt(theme_name, subtopic, batch_size, nc, source_lang, target_lang, sample_1, sample_2):
    """Build the prompt for a single batch, focused on a specific sub-topic."""

    # Length constraint based on NC
    if nc == "low":
        length_instruction = "Each source sentence should be around 10 to 15 tokens (words) long."
    elif nc == "medium":
        length_instruction = "Each source text should be around 30 tokens (words) long and can be multi-sentence."
    else:
        length_instruction = "Each source text should be around 60 tokens (words) long. It can be a short story or coherent paragraph."

    # Examples block
    examples_text = ""
    if sample_1 or sample_2:
        examples_text = f"\nHere are examples of the style and length expected for the {source_lang} text:\n"
        if sample_1:
    ...
    prompt = (
        ...
        f"Task: Generate exactly {batch_size} unique, coherent, and meaningful sentence pairs.\n"
        f"Each pair has a 'source' in {source_lang} and a 'target' in {target_lang}.\n"
...
def append_pairs_to_files(pairs, file_source, file_target):
    """Append validated pairs to the dataset files. Returns count of written pairs."""
    written = 0
    with open(file_source, "a", encoding="utf-8") as f_src, \
         open(file_target, "a", encoding="utf-8") as f_tgt:
        for pair in pairs:
            source = pair.get("source", "").replace('\\n', ' ').replace('\n', ' ').strip()
            target = pair.get("target", "").replace('\\n', ' ').replace('\n', ' ').strip()
...
def main():
    ...
    for t_idx, theme in enumerate(themes_to_process):
        ...
        # Calculate how many sentences per sub-topic
        ...
        for s_idx, subtopic in enumerate(subtopics):
            # Give the last sub-topic any remainder sentences
            ...
            while generated_for_subtopic < target_for_subtopic:
                ...
                # Add JSON format instruction for local models
                if engine == "LOCAL":
                    prompt += (
                        f"\nYou MUST return ONLY a valid JSON object containing a 'pairs' array.\n"
                        f"Each item must have 'source' and 'target' keys.\n"
                        f'Example: {{"pairs": [{{"source": "Hello", "target": "Olá"}}]}}'
                    )
                ...
                for attempt in range(1, CONFIG["MAX_RETRIES"] + 1):
                    if engine in ["API", "VERTEX"]:
                        pairs = generate_via_api(client, prompt, engine)
                        time.sleep(1)  # rate limit safety
                    else:
                        pairs = generate_via_local(prompt)
                ...
                print(f"    Batch done: +{written} pairs | Sub-topic progress: {generated_for_subtopic}/{target_for_subtopic} | Theme total: {theme_total_written}")

        print(f"\n  >>> Theme '{theme_name}' complete: {theme_total_written} total pairs written.\n")
